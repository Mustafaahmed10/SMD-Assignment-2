...
class Eleventh_Activity : AppCompatActivity() {
            ...
            userRef.addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        ...
                        userName2.text = "${name ?: "N/A"}"




                    } else {
                        Toast.makeText(this@Eleventh_Activity, "User data not found", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onCancelled(error: DatabaseError) {
                    Toast.makeText(this@Eleventh_Activity, "Error: ${error.message}", Toast.LENGTH_SHORT).show()
                }
            })
        }
